...
class TestLoadDatabase:
    ...
    def test_load_database_success(self, sample_data):
        ...
        with patch.object(Path, "open", return_value=mock_file_handle):
            result = load_database(
                sample_database_dir=Path("/mock/path"), nano_version="v12"
            )
    ...
    def test_load_database_uses_cache(self, sample_data):
        ...
        with patch.object(
            Path, "open", return_value=mock_file_handle
        ) as mock_open_file:
            # Call twice
...
class TestAddDatasetStr:
    ...
    def test_add_dataset_single_nick(
...
class TestAddDatasetList:
    ...
    def test_add_dataset_multiple_nicks(self, mock_campaign, mock_settings):
        ...
        with patch("xyh.config.campaigns.util.load_database") as mock_load_db:
            ...
            dataset = add_dataset(
                mock_campaign, "combined_dataset", ["sample1", "sample2"]
            )
    ...
    def test_add_dataset_xsec_tolerance_check(
    ...
    def test_add_dataset_xsec_mismatch_raises_error(
        self, mock_campaign, mock_settings
    ):
    ...
    def test_add_dataset_generator_weight_mismatch_raises_error(
        self, mock_campaign, mock_settings
    ):
        ...
        with patch("xyh.config.campaigns.util.load_database") as mock_load_db:
            ...
            with pytest.raises(
                ValueError, match="Values of 'generator_weight' not equal"
            ):
                add_dataset(
                    mock_campaign, "combined_dataset", ["sample1", "sample2"]
                )

    def test_add_dataset_generator_weight_type_mismatch_raises_error(
        self, mock_campaign, mock_settings
    ):
        ...
        with patch("xyh.config.campaigns.util.load_database") as mock_load_db:
            ...
            with pytest.raises(
                TypeError
            ):  # , match="Types of 'generator_weight' not equal"):
                add_dataset(
                    mock_campaign, "combined_dataset", ["sample1", "sample2"]
                )

    def test_add_dataset_era_mismatch_raises_error(
        self, mock_campaign, mock_settings
    ):
    ...
    def test_add_dataset_sample_type_mismatch_raises_error(
        self, mock_campaign, mock_settings
    ):
        ...
        with patch("xyh.config.campaigns.util.load_database") as mock_load_db:
            ...
            with pytest.raises(
                ValueError, match="Values of 'sample_type' not equal"
            ):
                add_dataset(
                    mock_campaign, "combined_dataset", ["sample1", "sample2"]
                )
